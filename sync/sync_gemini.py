...
if __name__ == "__main__":
    enviar_mensaje_mqtt("Mensaje de sincronización para Gemini")
    pregunta = "Hola Gemini, ¿cuál es el clima hoy?"
    respuesta_http = enviar_mensaje_http("http://localhost:5000/sync/gemini", pregunta)
    print(f"Respuesta de Gemini: {respuesta_http}")

    # Guardar un resumen después de una conversación
    nuevo_resumen = "Resumen de una nueva conversación importante."
    save_any_data('gemini', identidad_prompt, nuevo_resumen, color_favorito, mascota, tipo_de_modelo, version_modelo, habilidades_principales, limitaciones_conocidas, tono_de_voz, estilo_de_respuesta)
